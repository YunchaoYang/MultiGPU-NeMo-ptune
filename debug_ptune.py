...
# used for debugging multiGPU with SLURM - Yunchao Yang
...
@hydra_runner(config_path="conf", config_name="p_tuning_config_tp2")
def main(cfg) -> None:
    logging.info("\n\n************** Experiment configuration ***********")
    logging.info(f'\n{OmegaConf.to_yaml(cfg)}')

    megatron_amp_o2 = cfg.model.get('megatron_amp_O2', False) # mix precision related 

    plugins = []
    strategy = NLPDDPStrategy(no_ddp_communication_hook=True, find_unused_parameters=False,) # check this, crucial for process group
    ...
    print("!!!!check cluster_type: ", cfg.get('cluster_type', None) ) # None
    ...
    ##############################################################################################################
    # debug
    print("!!! check: ", cfg.model.get("restore_path", None))
    
    print("!!! check existing_tasks: ")
    print(cfg.model.existing_tasks)
    print("!!! check task_templates: ")
    print( cfg.model.task_templates)
    print("!!! check new_tasks: ")
    print(cfg.model.new_tasks)
    ...
    print("check exp_manager: ")
    print(OmegaConf.to_yaml(cfg.exp_manager))
    print("\n"*3)
    
    OmegaConf.save(cfg, "tp2-config.yaml")
    ##############################################################################################################
    ##############################################################################################################
